...
public class WrapperTriggerPaaSBean {
    ...
    public WrappedIaasHealthByTrigger getTriggerWrapper(String serverType, List<ZabbixMonitoredHostResponseV2_4> hosts,
	    String groupName) throws ZabbixException {
	...
	for (ZabbixMonitoredHostResponseV2_4 host : hosts) {
	    ArrayList<ZabbixItemResponse> triggers = zabAdapClientSetter.getTriggerWithProblemsService(serverType,
		    host.getHostid(), null, ZabbixMethods.TRIGGER.getzabbixMethod(), null);
		...
		if (zabbixItemResponse.getLastchange() != null) {
		     ...
		     * trigger shot not before than 5mins ago are needed to be
