...
import it.prisma.domain.dsl.monitoring.pillar.wrapper.DescriptionEvent;
...
public class ZabbixEventBean {
    ...
    public MonitPillarEventResponse getEvent(String serverType, List<ZabbixMonitoredHostResponseV2_4> hosts,
	    String hostId, List<ZabbixHostGroupResponse> groups, String groupId, String tag_service,
	...
	ArrayList<ZabbixItemResponse> events = zabAdapPaas.getItemsService(serverType, hostId, null, null, null,
	...
	for (ZabbixItemResponse event : events) {
		...
		ArrayList<ZabbixItemResponse> items = zabAdapPaas.getItemsService(serverType, null, null, null,
		...
		for (ZabbixItemResponse item : items) {
			...
			if (requestTime != null && FilterTimeRequestHandlerMonitoring.checkDateFormat(requestTime)) {
				...
				wrappedEventDescritp.setClock(TimestampMonitoring.decodUnixTime2Date(Long
					.parseLong(event.getClock())));
