...
public class WrapperIaasHealthBean {
    ...
    public WrappedIaasHealthByTrigger getTriggerforIaasHealth(String testbed, String serverType, String group)
	...
	ArrayList<ZabbixItemResponse> triggers = zabAdapClientSetter.getTriggerWithProblemsService(serverType, null,
		null, ZabbixMethods.TRIGGER.getzabbixMethod(), group);

	for (ZabbixItemResponse zabbixItemResponse : triggers) {
	    // get the host associated to trigger
	    ArrayList<ZabbixMonitoredHostsResponse> hosts = zabAdapClientSetter.getHostsService(serverType,
		    ZabbixMethods.HOST.getzabbixMethod(), null, null, null, zabbixItemResponse.getTriggerid());
		...
		String timeFromZabbix = TimestampMonitoring.decodUnixTime2Date(Long.parseLong(zabbixItemResponse
			.getLastchange()));

		/*
		 * The next piece of commmented code is to be used whenever
		 * trigger shot not before than 5mins ago are needed to be taken
		 * in consideration
		 */
