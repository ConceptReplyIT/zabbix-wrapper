...
public class TemplateIDByName implements Serializable {
	...
	public List<String> getTemplateID(String serverType, List<String> services, Boolean activeMode)
		...
		for (String service : services) {
			...
			for (int j = 0; j < templateList.size(); j++) {
				...
				if (config.isCeilometerScriptUsed()) {
					if (templateName
							.equalsIgnoreCase(templateMode + config.TemplateIaaSCeilometerNameinZabbixMetrics())
				...
				if (templateName.equalsIgnoreCase(templateMode + service)
						|| (service.contains(MonitoringConstant.ATOMIC_SERVICE_AGGREGATOR) && templateName
								.equalsIgnoreCase(ZabbixConstant.TEMPLATE_AGGREGATOR))) {
					templateIDs.add(templateList.get(j).getTemplateid());
		...
		if (templateIDs.size() - 1 != services.size()
				&& (serverType.equalsIgnoreCase(InfoType.SERVICE.getInfoType()) && !services.get(0).contains(
						MonitoringConstant.ATOMIC_SERVICE_AGGREGATOR))) {
			throw new NotFoundZabbixException("One of the passed Atomic Services: "
