package it.monitoringpillar.wrapper.zabbix.iaas;
...
public class WrapperIaaSBean implements Serializable {
     ...
     * @throws Exception
     */
    public MonitoringWrappedResponseIaas getWrappedIaas(String serverType, String iaasTypes, String iaashosts,
	    List<ZabbixHostGroupResponse> groups, String groupId, List<ZabbixMonitoredHostsResponse> hosts,
	    String hostId) throws ZabbixException {
	MonitoringWrappedResponseIaas wrappedIaas = new MonitoringWrappedResponseIaas();
	// wrappedIaas.setTestbed(testbedType);
	wrappedIaas
		.setIaasMachineGroups(getGroupInfo(serverType, iaasTypes, iaashosts, groups, groupId, hosts, hostId));
	return wrappedIaas;
    }
     ...
     * @throws IOException
     */
    private ArrayList<IaasGroupOfMachine> getGroupInfo(String serverType, String iaasGroupNames, String iaashostNames,
	    List<ZabbixHostGroupResponse> groups, String groupId, List<ZabbixMonitoredHostsResponse> hosts,
	    String hostId) throws ZabbixException {
	...
	for (ZabbixHostGroupResponse groupInfo : groups) {
		...
		hostgroupList.setIaasMachinesList(getMachinesList(serverType, hostGroupIdtoAdapter, hostIdtoAdapter,
			hosts));
		...
		hostgroupList.getIaasMachines().addAll(
			(getMachinesList(serverType, hostGroupIdtoAdapter, hostIdtoAdapter, hosts)));
     ...
     * @throws Exception
     */
    private ArrayList<IaasMachine> getMachinesList(String serverType, String hostGroupIdtoAdapter,
	...
	if (!(hostIdtoAdapter == null)) {
		...
		if (hostIdtoAdapterCurrent.equals(hostIdtoAdapter)) {
		    ...
		    ArrayList<ZabbixItemResponse> zabbixinterfaceResponse = (ArrayList<ZabbixItemResponse>) zabAdapClientSetter
			    .getZabbixFeatureMultiService(serverType, hostIdtoAdapter, null,
		    ...
		    machines.setMetrics(getIaasMetrics(serverType, hostIdtoAdapter));
     ...
     * @throws Exception
     */
    private ArrayList<IaaSMetric> getIaasMetrics(String serverType, String hostidtoAdapter) throws ZabbixException {
	...
	ArrayList<ZabbixItemResponse> metrics4Host = (ArrayList<ZabbixItemResponse>) zabAdapClientSetter
		.getItemsService(serverType, hostidtoAdapter, templateId, null, null,
	...
	for (ZabbixItemResponse metrictype : metrics4Host) {
		...
		try {
		    PrismaIaasScript scriptIaas = (PrismaIaasScript) JsonUtility.deserializeJson(
			    metricValue.toString(), PrismaIaasScript.class);
		...
		try {
		    HypervisorGroup hypervisorScript = (HypervisorGroup) JsonUtility.deserializeJson(
			    metricValue.toString(), HypervisorGroup.class);
	    ...
	    iaasMetrics.setIaasThresholds(getThreshold(serverType, metricID));
     ...
     * @throws Exception
     */
    private List<IaasThresholdsList> getThreshold(String serverType, String metricID) throws ZabbixException {

	ArrayList<ZabbixItemResponse> trigger4Host = (ArrayList<ZabbixItemResponse>) zabAdapClientSetter
		.getTriggerService(serverType, hostIdtoAdapter, metricID, ZabbixMethods.TRIGGER.getzabbixMethod());
