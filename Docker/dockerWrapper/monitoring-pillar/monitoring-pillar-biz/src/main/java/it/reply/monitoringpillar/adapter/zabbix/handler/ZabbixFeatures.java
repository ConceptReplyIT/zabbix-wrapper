...
public class ZabbixFeatures {
	...
	public enum ZabbixMethods {
		HOSTCREATE("host.create"), HOSTGROUPCREATE("hostgroup.create"), HOSTDELETE("host.delete"), HOSTGROUPUPDATE(
				"hostgroup.update"), GROUPDELETE("hostgroup.delete"), HOST("host.get"), HOSTGROUP(
						"hostgroup.get"), TEMPLATE("template.get"), INTERFACE("hostinterface.get"), METRIC(
								"item.get"), TRIGGER("trigger.get"), HISTORY("history.get"), EVENT(
										"event.get"), MASSUPDATE("host.massupdate"), HOSTUPDATE(
												"host.update"), ITEMUPDATE("item.update"), TREND("trend.get"), GETPROXY(
														"proxy.get"), CREATEPROXY("proxy.create"), DELETEPROXY(
																"proxy.delete"), GETUSERS("user.get"), CREATEUSER(
																		"user.create");
	...
	public enum ServerType {
		...
		SERVERIAAS("infrastructure"), SERVERMETRICS("service"), SERVERWATCHER("watcher"), PAAS("paas"), ALL("all");
