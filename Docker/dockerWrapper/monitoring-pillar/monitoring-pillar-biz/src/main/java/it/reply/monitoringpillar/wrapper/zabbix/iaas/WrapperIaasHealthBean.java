...
public class WrapperIaasHealthBean {
	...
	public WrappedIaasHealthByTrigger getTriggerforIaasHealth(String zone, String serverType, String group)
		...
		ArrayList<ZabbixItemResponse> triggers = zabAdapClientSetter.getTriggerWithProblemsService(zone, serverType,
				null, null, ZabbixMethods.TRIGGER.getzabbixMethod(), group);

		for (ZabbixItemResponse zabbixItemResponse : triggers) {
			// get the host associated to trigger
			ArrayList<ZabbixMonitoredHostsResponse> hosts = zabAdapClientSetter.getHostsService(zone, serverType,
